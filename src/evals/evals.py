...
# 1. Define the Project Root relative to THIS file's location (src/evals/run_evals.py)
# .parent (evals) -> .parent (src) -> .parent (project root)
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent

# 2. Map all strictly relative directories
...
def load_local_context():
    ...
    try:
        with open(roster_path, "r", encoding="utf-8") as f:
            ...
            # Extract just the string names from the complex dictionary
            roster = [person["name"] for person in raw_roster.get("people", [])]
        
        # If past_decision.md doesn't exist yet, default to empty string
        past_decisions = ""
        if decisions_path.exists():
            with open(decisions_path, "r", encoding="utf-8") as f:
                past_decisions = f.read()
                
        return roster, past_decisions
...
def run_evaluation():
    ...
    for eval_data in gold_standards:
        ...
        # Load transcript using the exact ID from the JSON
        ...
        time.sleep(1) # Prevent rate limiting between requests
...
"""
    ...
    # Ensure reports directory exists, then write the file
